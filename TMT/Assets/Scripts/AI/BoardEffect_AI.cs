...
public class BoardEffect_AI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        PinkEffectArray[0] = -200;
        PinkEffectArray[1] = -200;
        PinkEffectArray[2] = -300;
        PinkEffectArray[3] = -300;
    ...
    void Update()
    {
        if ((TurnSystem_AI.TurnProcess == 3) && (EffectApplyGoingOn == false))
        {
            EffectApplyGoingOn = true;
            if ( ((TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber) && (MovementSystem_AI.Player1CurrentBoard == 0))  || ((TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player2TurnNumber) && (MovementSystem_AI.Player2CurrentBoard == 0))) //스타트 칸은 보드 효과 없음
            {
                EffectApplyGoingOn = false;
                TurnSystem_AI.TurnProcess = 4;
            }
            else
            {
                CardChoice(RouletteManager_AI.RouletteColor);
            }
        }
    }
    ...
    public void CardChoice(int Color)
    {
        if (Color <= 4) //빨강(식비,간식) 주황(문화생활,여행) 노랑(사회생활) 초록(백화점,마트)
        {
            ...
            if (TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber)
                CardChoicePopUp(Color, MovementSystem_AI.Player1CurrentLineNumber, Choice1, Choice2, 1);
            else
                CardChoicePopUp(Color, MovementSystem_AI.Player2CurrentLineNumber, Choice1, Choice2, 2);
        }
        ...
        {
            if (TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber)
                RandomChoice = MovementSystem_AI.Player1CurrentBoard;
            else
                RandomChoice = MovementSystem_AI.Player2CurrentBoard;

            if (RandomChoice == 6)
            {
                RandomChoice = 0;
                if (TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber)
                    RandomChoicePopUp(Color, 1, 0, 1);
                else
                    RandomChoicePopUp(Color, 1, 0, 2);
            }
            else if (RandomChoice == 13)
            {
                RandomChoice = 1;
                if (TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber)
                    RandomChoicePopUp(Color, 2, 1, 1);
                else
                    RandomChoicePopUp(Color, 2, 1, 2);
            }
            else if (RandomChoice == 20)
            {
                RandomChoice = 2;
                if (TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber)
                    RandomChoicePopUp(Color, 3, 2, 1);
                else
                    RandomChoicePopUp(Color, 3, 2, 2);
            }
            else if (RandomChoice == 27)
            {
                RandomChoice = 3;
                if (TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber)
                    RandomChoicePopUp(Color, 4, 3, 1);
                else
                    RandomChoicePopUp(Color, 4, 3, 2);
            }
        }
        else if (Color == 7) //보라(찬스)
        {
            ...
            if (TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber)
                RandomChoicePopUp(Color, MovementSystem_AI.Player1CurrentLineNumber, RandomChoice, 1);
            else
                RandomChoicePopUp(Color, MovementSystem_AI.Player2CurrentLineNumber, RandomChoice, 2);
        }
    ...
    public void CardChoicePopUp(int Color, int LineNumber, int Choice1, int Choice2, int Who) //카드 두개 중에 선택할 수 있는 경우
    {
        ...
        if (Who == 1)
            ...
            StartCoroutine(CardSelectionForAI(Choice1, Choice2));
    }
    ...
    public void RandomChoicePopUp(int Color, int LineNumber, int RandomChoice, int Who)
    {
        ...
        if (Who == 1)
            ...
            StartCoroutine(RandomCardSelectionForAI(RandomChoice));
    }
    ...
    IEnumerator CardPopUp() //선택 카드(2개) 점점 커지면서 나타나기
    {
        ...
        this.GetComponent<AudioSource>().Play();
        if (TurnSystem_AI.CurrentTurnNumber == TurnSystem_AI.Player1TurnNumber) //사용자(플레이어1) 턴이라면 카드 선명하게
        {
    ...
    IEnumerator CardChoiceTimerStart(int Who, int Choice1, int Choice2) //타이머 보여줌
    {
        ...
        if (Choice1OkButton.IsInteractable() || Choice2OkButton.IsInteractable()) //시간 지나도 안골랐다면
            ChoiceResultShow(1, 1, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player1CurrentLineNumber, Choice1, MovementSystem_AI.Player1CurrentBoard);
    }
    ...
    IEnumerator RandomCardChoiceTimerStart(int Who, int RandomChoice) //타이머 보여줌
    {
        ...
        if (RandomChoiceOkButton.IsInteractable()) //시간 지나도 안골랐다면
            RandomChoiceOk(1, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player1CurrentLineNumber, RandomChoice, MovementSystem_AI.Player1CurrentBoard);
    }

    IEnumerator CardSelectionForAI(int Choice1, int Choice2)
    {
        yield return new WaitForSeconds(4.0f);

        if (RouletteManager_AI.RouletteColor == 1) //빨강
        {
            if(( MovementSystem_AI.Player2CurrentLineNumber % 2) != 0) //1, 3 라인(식사)
            {
                if (MoneySystem_AI.Player2PossessMoney <= 70000)
                    ChoiceResultShow(1, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice1, MovementSystem_AI.Player2CurrentBoard);
                else if (TirednessSystem_AI.Player2Health <= 150)
                    ChoiceResultShow(2, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice2, MovementSystem_AI.Player2CurrentBoard);
                else
                    ChoiceResultShow(1, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice1, MovementSystem_AI.Player2CurrentBoard);
            }
            else //2,4 라인(간식)
            {
                if (MoneySystem_AI.Player2PossessMoney <= 25000)
                    ChoiceResultShow(2, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice2, MovementSystem_AI.Player2CurrentBoard);
                else if (TirednessSystem_AI.Player2Health <= 150)
                    ChoiceResultShow(1, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice1, MovementSystem_AI.Player2CurrentBoard);
                else
        ...
        else if(RouletteManager_AI.RouletteColor == 2) //주황
        {
            if ((MovementSystem_AI.Player2CurrentLineNumber % 2) != 0) //1, 3 라인(문화생활)
            {
                if (MoneySystem_AI.Player2PossessMoney <= 160000)
                    ChoiceResultShow(1, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice1, MovementSystem_AI.Player2CurrentBoard);
                else if (TirednessSystem_AI.Player2Health <= 150)
                    ChoiceResultShow(2, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice2, MovementSystem_AI.Player2CurrentBoard);
                else
            ...
            {
                if (MoneySystem_AI.Player2PossessMoney <= 900000)
                    ChoiceResultShow(1, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice1, MovementSystem_AI.Player2CurrentBoard);
                else if (TirednessSystem_AI.Player2Health <= 150)
                    ChoiceResultShow(2, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice2, MovementSystem_AI.Player2CurrentBoard);
                else
        ...
        else if (RouletteManager_AI.RouletteColor == 3) //노랑
        {
            if (MoneySystem_AI.Player2PossessMoney <= 160000)
                ChoiceResultShow(1, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice1, MovementSystem_AI.Player2CurrentBoard);
            else if (TirednessSystem_AI.Player2Health <= 150)
        ...
        else if (RouletteManager_AI.RouletteColor == 4) //초록
        {
            if ((MovementSystem_AI.Player2CurrentLineNumber % 2) != 0) //1, 3 라인(백화점)
            {
                if (MoneySystem_AI.Player2PossessMoney <= 350000)
                    ChoiceResultShow(1, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice1, MovementSystem_AI.Player2CurrentBoard);
                else if (TirednessSystem_AI.Player2Health <= 150)
                    ChoiceResultShow(2, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice2, MovementSystem_AI.Player2CurrentBoard);
                else
        ...
        else if (RouletteManager_AI.RouletteColor == 5) //아르바이트(수입)
        {
            if(TirednessSystem_AI.Player2Health <= 235)
                ChoiceResultShow(1, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice1, MovementSystem_AI.Player2CurrentBoard);
            else
                ChoiceResultShow(2, 2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, Choice2, MovementSystem_AI.Player2CurrentBoard);
        }
    ...
    IEnumerator RandomCardSelectionForAI(int RandomChoice)
    {
        yield return new WaitForSeconds(3.0f);
        RandomChoiceOk(2, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player2CurrentLineNumber, RandomChoice, MovementSystem_AI.Player2CurrentBoard);
    }
    ...
    {
        ChoiceResultShow(2, 1, RouletteManager_AI.RouletteColor, MovementSystem_AI.Player1CurrentLineNumber, Choice2, MovementSystem_AI.Player1CurrentBoard);
    }
    ...
    public void ChoiceResultShow(int LeftOrRight, int Who, int Color, int LineNumber, int Choice, int BoardLocation)
    {
        ...
        Choice2OkButton.interactable = false;
        CardChoiceTimer.SetActive(false);
        if (Who == 1) //사용자(플레이어1) 턴이라면 카드 선명하게
        {
        ...
        EffectApply_AI.HealthEffectApplyingGoingOn = true;
    ...
    public void RandomChoiceOk(int Who, int Color, int LineNumber, int Choice, int BoardLocation)
    {
        ...
        if (Color == 6) //분홍
        {
            EffectApply_AI.MoneyEffectApplyingGoingOn = false;
            StartCoroutine(EA.HealthChangeApply(Who, PinkEffectArray[Choice]));
        }
        else if (BoardLocation == 0) //용돈
        {
            EffectApply_AI.HealthEffectApplyingGoingOn = false;
    ...
    IEnumerator WaitForChangeApply(int Who)
    {
        ...
        while ((EffectApply_AI.MoneyEffectApplyingGoingOn == true) || (EffectApply_AI.HealthEffectApplyingGoingOn == true))
        {
            yield return null;
        }

        if (TurnSystem_AI.TurnProcess == 2)//용돈의 경우
        {
            BonusGoingOn = false;
        }
        else if (TurnSystem_AI.TurnProcess == 3)//일반 보드이펙트 경우
        {
            EffectApplyGoingOn = false;
            TurnSystem_AI.TurnProcess = 4;
        }
